...
// only the admin has access to serven_on
int server_on               = 1; // no further connections when serven_on =0
int number_of_accounts      = 0; // number of accounts present in the database
int logged[1024]  = {0}; // account id's which are logged in, a total of 1024 connections concurrently

void reverse(char* begin, char* end) { // reverses a string
	char aux; 
	while(end>begin)
...
// functions to access the logged global variable, this has to be shared between multiple threads and hence, appropriate locking has been defined for reading
int open_connection(int id) // this takes in an account id and enters its value in the logged array indicating that the account id is active
{
...
int close_connection(int id) // this takes in an accunt_id and removes its value from the logged indicating that the account is now logging out
{
...
// routing all the functions to access logged from here so that all of them can be simultaneously mutex-locked
int access_connection(int id, int what)
{
    int out = -1;
    pthread_mutex_lock(&mutex_lock1);
    if(what == 1)
        out = open_connection(id);
    else if(what == -1)
        out = close_connection(id);
    else 
        out = is_open(id);
...
void database_init(void) // this initializes our personal data-store. It creates an admin account and 10 other accounts in which 5 are single-user accounts and 5 are joint-user accounts
{
...
int match_password(account_t* pointer, char* pwd) // password verification for an account
{
    int i = 0;
    int a;
    int b;
...
int get_len(int* arr) // know the number of account ids that are associated to an account
{
    int counter = 0;
    for(int i = 0; i < ACC; i++)
    {
...
void get_info(unsigned int id, account_t** pointer) // takes in the account id and returns the info; if the account is not present, the value of attribute "this_id" is set to 0
{
...
int manage_balance(float new, int id) // takes in the amount which is to be added to the balance and the corresponding id of the acccount, and does the needfull
{
    struct flock lock; // putting a read lock on the file
...
int delete_account(int id) // takes an id and deletes that account
{
    struct flock lock; // putting a read lock on the file
...
int add_account(account_t* new) //adds new account into the database
{
    struct flock lock; // putting a read lock on the file
...
void* login(void* nsd_) // all server functionalities
{
    char id[2]           = {0};
    ...
    while(1)
    {
        char write_buffer[1024] = {0}; // send messages to the client
        char read_buffer[1024]  = {0}; // receive messages from the client
        ...
        if(i == 0) // iteration 1, here the login id is verified
        {
            ...
            if(pointer->this_id == 0)            // login id was not found
                tag = -1;
            if(access_connection(pointer->this_id, 0) == 0)   // login id added to the open connections
            {
                if(tag == -1)
        ...
        else // main loop (can only be reached after a successful login)
        {
            while(1)
            {
                if(open_id != 1) // this is a user account (id == 1 has been reserved for the admin)
                {
                    ...
                    if((int)read_buffer[0] == 54 && (int)read_buffer[1] == 10) // the user wants to logout
                    {
                        access_connection(open_id, -1);
                        close(nsd);
                    ...
                    else if((int)read_buffer[0] == 51 && (int)read_buffer[1] == 10) // the user wants to know the account balance
                    {
                    ...
                    else if((int)read_buffer[0] == 53 && (int)read_buffer[1] == 10) //the user wants to know the account details
                    {
                        get_info(open_id, &pointer);
                        ...
                        if(get_len(pointer->id) == 1)
                                strcpy(write_buffer, "This account  is a single account");
                            else
                                strcpy(write_buffer, "This account is a joint account");
                        send(nsd, write_buffer, sizeof(write_buffer), MSG_CONFIRM);
                    ...
                    else if((int)read_buffer[0] == 49 && (int)read_buffer[1] == 10) // the user wants to deposit 
                    {
                        memset(write_buffer, 0, sizeof(write_buffer));
                        ...
                        manage_balance(strtof(read_buffer, NULL), open_id);
                    }
                    else if((int)read_buffer[0] == 50 && (int)read_buffer[1] == 10) // the user wants to withdraw
                    {
                        memset(write_buffer, 0, sizeof(write_buffer));
                        strcpy(write_buffer, "Enter the amount: ");
                        send(nsd, write_buffer, sizeof(write_buffer), MSG_CONFIRM);
                        ...
                        memset(write_buffer, 0, sizeof(write_buffer));
                        if(manage_balance(-1*(strtof(read_buffer, NULL)), open_id) == -2)
                            strcpy(write_buffer, "1");
                        else
                            strcpy(write_buffer, "0");
                        send(nsd, write_buffer, sizeof(read_buffer), MSG_CONFIRM);
                    }
                    else if((int)read_buffer[0] == 52 && (int)read_buffer[1] == 10) // the user wants to change the account password
                    {
                        memset(write_buffer, 0, sizeof(write_buffer));
                ...
                else if(open_id == 1) // admin
                {
                    ...
                    recv(nsd, read_buffer, sizeof(read_buffer), 0);
                    if((int)read_buffer[0] == 53 && (int)read_buffer[1] == 10) // the admin wishes to logout
                    {
                        access_connection(open_id, -1);
                        close(nsd);
                    ...
                    else if((int)read_buffer[0] == 51 && (int)read_buffer[1] == 10) // the admin wishes to get information about a particular account
                    {
                        memset(write_buffer, 0, sizeof(write_buffer));
                    ...
                    else if((int)read_buffer[0] == 50 && (int)read_buffer[1] == 10) // the admin wants to delete an account
                    {
                        memset(write_buffer, 0, sizeof(write_buffer));
                        strcpy(write_buffer, "Enter the account id: ");
                        send(nsd, write_buffer, sizeof(write_buffer), MSG_CONFIRM);
                    ...
                    else if((int)read_buffer[0] == 49 && (int)read_buffer[1] == 10) // the admin wants to create an account
                    {
                        memset(write_buffer, 0, sizeof(write_buffer));
                    ...
                    else if((int)read_buffer[0] == 52 && (int)read_buffer[1] == 10) //the admin wishes to modify the account password
                    {
                        memset(write_buffer, 0, sizeof(write_buffer));
                        strcpy(write_buffer, "Enter the account id: ");
                        send(nsd, write_buffer, sizeof(write_buffer), MSG_CONFIRM);
                    ...
                    else if((int)read_buffer[0] == 54 && (int)read_buffer[1] == 10) // the admin wishes to close the server
                    {
...
{   
    ...
    {            
        ...
        else
        {
            ...
            if(counter == 1024)
            {
                write(2, "Maximum thread count exceeded!\n", strlen("Maximum thread count exceeded!\n"));
            }
        }
    }   
    ...
    shutdown(nsd, SHUT_RDWR); // server down
